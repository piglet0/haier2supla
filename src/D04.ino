...
//#include <supla/log_wrapper.h>  // not required, logging is available by default in SuplaDevice
...
Supla::Control::HaierVirtualRelay *haierSwingBothRelay = nullptr;
...
Supla::Sensor::VirtualBinary *haierSwingBoth = nullptr;
...
// config button to reset WiFi and other settings
  ...
  //esp_log_level_set(SUPLA_TAG, ESP_LOG_DEBUG);
  //#endif
...
// TODO how to reverse action?    
  ...
  if (interfaceLvl >= 2) {
    ...
    haierSwingBothRelay = new Supla::Control::HaierVirtualRelay(
      std::bind(&HaierSmartair2Controller::setSwingBoth, &haierController, std::placeholders::_1),
      std::bind(&HaierSmartair2Controller::getSwingBoth, &haierController)
    );
    haierSwingBothRelay->setDefaultFunction(SUPLA_CHANNELFNC_POWERSWITCH);
    snprintf(caption, sizeof(caption), "%s-Swing Both", roomName);
    haierSwingBothRelay->setInitialCaption(caption);
    ...
    haierSwingVerticalRelay->setInitialCaption(caption);
      ...
      // SUPLA channel: binary sensor indicating Swing Both
      haierSwingBoth = new Supla::Sensor::VirtualBinary;
      haierSwingBoth->setDefaultFunction(SUPLA_CHANNELFNC_BINARY_SENSOR);
      snprintf(caption, sizeof(caption), "%s-Swing Both", roomName);
      haierSwingBoth->setInitialCaption(caption);
...
void loop() {
  ...
  if (haierSwingBothRelay != nullptr) {
    haierSwingBothRelay->syncState();
  }
  ...
  if (haierSwingBoth != nullptr) {
    haierController.getSwingBoth() ? haierSwingBoth->set() : haierSwingBoth->clear();
  }
